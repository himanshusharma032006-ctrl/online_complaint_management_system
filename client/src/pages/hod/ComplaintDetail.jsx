...
import { ArrowLeft, MessageSquare, Send, Calendar, Tag, AlertCircle, CheckCircle2, UserCheck, Shield } from 'lucide-react'
...
export default function HodComplaintDetail() {
  const { id } = useParams()
  ...
  // Fetch Department Staff Coordinators
  ...
  const commentMutation = useMutation({
    ...
    onSuccess: () => {
      setCommentText('')
      setIsInternal(false)
      queryClient.invalidateQueries(['hod-comments', id])
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (updated) => {
      queryClient.invalidateQueries(['hod-complaint', id])
      queryClient.invalidateQueries(['hod-stats'])
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ type: 'success', title: 'Coordinator Assigned', message: 'Staff coordinator assigned successfully.' })
    },
    onError: (err) => {
  ...
  if (error || !complaint) {
    return (
      <div className="bg-red-50 dark:bg-red-950/20 text-red-700 dark:text-red-300 p-4 rounded-xl flex items-center gap-2">
        ...
        <span>Failed to load complaint details. Please try again.</span>
      </div>
    )
  }
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800 rounded-xl p-6 shadow-sm">
              ...
              {isLoadingComments ? (
                ...
                comments.map((comm) => (
                  <div key={comm.id} className="flex gap-3 items-start text-sm">
                    ...
                    <div className={`flex-1 border rounded-xl p-3.5 ${
                      comm.is_internal
            ...
            <form onSubmit={handleCommentSubmit} className="space-y-3">
              <textarea
                ...
                placeholder="Type a response or add internal note..."
              ...
              <div className="flex justify-between items-center">
                <label className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 cursor-pointer select-none">
                  ...
                  />
                  Internal Note (Visible only to staff & HODs)
                </label>
                <button
                  type="submit"
            ...
            {isAssigning ? (
              <form onSubmit={handleAssignSubmit} className="space-y-4">
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 border border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-805 text-gray-600 dark:text-gray-400 text-xs font-semibold py-2 rounded-xl transition-all"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <div className="space-y-4">
                ...
                {['resolved', 'rejected', 'closed'].includes(complaint.status) ? (
                  <div className="text-xs text-gray-450 border-t border-gray-100 dark:border-gray-800 pt-3">
                    Assignment locked as complaint is complete.
                  </div>
                ) : (
                  <button
                    onClick={() => setIsAssigning(true)}
                    className="w-full border border-purple-200 hover:bg-purple-50/50 dark:border-purple-900/40 dark:hover:bg-purple-950/20 text-purple-600 dark:text-purple-400 text-xs font-semibold py-2.5 rounded-xl transition-all"
                  >
              ...
              <form onSubmit={handleStatusSubmit} className="space-y-4">
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 border border-gray-200 dark:border-gray-805 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400 text-xs font-semibold py-2 rounded-xl transition-all"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <div className="space-y-4">
                ...
                {complaint.resolution_note && (
                  <div className="text-sm border-t border-gray-100 dark:border-gray-800 pt-3">
                    ...
                    <p className="text-gray-700 dark:text-gray-300 italic">"{complaint.resolution_note}"</p>
                  </div>
                )}
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800 rounded-xl p-6 shadow-sm">
            ...
            {complaint.is_anonymous === 1 ? (
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-500 flex items-center justify-center font-bold text-sm">
                  ?
                </div>
                <div>
                  <h4 className="font-semibold text-gray-855 dark:text-gray-205 text-sm">Anonymous</h4>
                  <p className="text-xs text-gray-400 dark:text-gray-500">Contact details hidden</p>
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-gray-855 dark:text-gray-205 text-sm">
                ...
                {complaint.enrollment_no && (
                  <div className="text-xs text-gray-500 dark:text-gray-400 border-t border-gray-105 dark:border-gray-800 pt-2 flex justify-between">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800 rounded-xl p-6 shadow-sm">
              ...
              <div className="space-y-2">
                ...
                {complaint.feedback.comment && (
                  <p className="text-xs italic text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-850 p-3 rounded-lg border border-gray-100 dark:border-gray-800">
                    "{complaint.feedback.comment}"
                  </p>
                )}
              </div>
