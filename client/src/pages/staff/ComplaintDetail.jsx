...
import { ArrowLeft, MessageSquare, Send, Calendar, Tag, AlertCircle, Info, CheckCircle2 } from 'lucide-react'
...
export default function StaffComplaintDetail() {
  ...
  // Fetch Complaint (timeline, feedback populated by backend)
  ...
  const commentMutation = useMutation({
    ...
    onSuccess: () => {
      setCommentText('')
      setIsInternal(false)
      queryClient.invalidateQueries(['staff-comments', id])
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (updated) => {
      queryClient.invalidateQueries(['staff-complaint', id])
  ...
  if (error || !complaint) {
    return (
      <div className="bg-red-50 dark:bg-red-950/20 text-red-700 dark:text-red-300 p-4 rounded-xl flex items-center gap-2">
        ...
        <span>Failed to load complaint. Please return to dashboard and try again.</span>
      </div>
    )
  }
            ...
            {isUpdatingStatus ? (
              <form onSubmit={handleStatusSubmit} className="space-y-4">
                ...
                {['resolved', 'rejected'].includes(status) && (
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Explain how the issue was resolved or why it was rejected..."
                      className="w-full px-3 py-2 border border-gray-200 dark:border-gray-800 rounded-xl bg-transparent text-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none"
                    />
                  </div>
                )}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 border border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400 text-xs font-semibold py-2 rounded-xl transition-all"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <div className="space-y-4">
                ...
                {['resolved', 'rejected', 'closed'].includes(complaint.status) ? (
                  <div className="bg-green-50 dark:bg-green-950/20 text-green-700 dark:text-green-300 p-3.5 rounded-xl text-xs flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 flex-shrink-0" />
                    <span>This complaint has been completed and marked as {complaint.status}.</span>
                  </div>
                ) : (
                  <button
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-150 dark:border-gray-800 rounded-xl p-6 shadow-sm">
            ...
            {complaint.is_anonymous === 1 ? (
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-gray-850 dark:text-gray-200 text-sm">Anonymous</h4>
              ...
              <div className="space-y-3">
                ...
                {complaint.enrollment_no && (
                  <div className="text-xs text-gray-500 dark:text-gray-400 border-t border-gray-105/60 dark:border-gray-800/60 pt-2 flex justify-between">
